		...
		<form method="POST" enctype="multipart/form-data" action="{{ route('categories.update', $category->id) }}">
			...
			<div class="card mb-4 shadow-sm" style="border-radius: 12px; overflow: hidden; border: 1px solid #007bff;">
				<div class="card-header text-white py-3" style="background: linear-gradient(135deg, #0056b3, #007bff);">
					<h5 class="card-title text-white mb-1"><i class="fas fa-box mr-2"></i> Packaging Details</h5>
					<small class="text-white-50 font-weight-normal d-block">Update the packaging unit name and detailed descriptions for item categorization.</small>
				</div>
				<div class="card-body p-4">
					<div class="row mb-3">
						<div class="col-lg-6">
							<div class="form-group mb-0">
								<label class="font-weight-bold">Packaging Name <span class="text-danger">*</span></label>
								<div class="custom-autocomplete-wrapper position-relative">
									<input class="form-control" type="text" name="name" id="category_name_input" autocomplete="off" placeholder="Search or type packaging name..." value="{{ old('name', $category->name) }}">
									<div id="category_autocomplete_dropdown" class="w-100 position-absolute shadow bg-white" style="display: none; max-height: 200px; overflow-y: auto; z-index: 1000; border-radius: 10px; top: 100%; left: 0; margin-top: 5px; border: 1px solid #ced4da;">
										<ul class="list-unstyled mb-0" id="category_autocomplete_list"></ul>
					...
					<div class="row">
						<div class="col-lg-12">
							<div class="form-group mb-0">
								...
								<textarea class="form-control service-desc" name="description" rows="4" style="resize: vertical;" placeholder="Enter description...">{{ old('description', $category->description) }}</textarea>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="card mb-4 shadow-sm" style="border-radius: 12px; border: 1px solid #e3e8ee;">
				<div class="card-body p-3 d-flex justify-content-between align-items-center">
					<a href="{{route('categories.index')}}" class="btn btn-secondary btn-lg rounded-pill px-4"><i class="fas fa-arrow-left mr-1"></i> Cancel</a>
					<button class="btn btn-primary btn-lg rounded-pill px-5 shadow-sm" type="submit"><i class="fas fa-sync-alt mr-1"></i> Update Packaging</button>
				</div>
			</div>
		</form>
	</div>			
...
$(document).ready(function() {
    ...
    const $input = $('#category_name_input');
    const $dropdown = $('#category_autocomplete_dropdown');
    const $list = $('#category_autocomplete_list');
    ...
    function fetchAutocomplete(term, page, append = false) {
        ...
        $.ajax({
            url: "{{ route('categories.autocomplete') }}",
