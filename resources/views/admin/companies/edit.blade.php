		...
		<form method="POST" enctype="multipart/form-data" action="{{ route('companies.update', $company->id) }}">
			...
			<div class="card mb-4 shadow-sm" style="border-radius: 12px; overflow: hidden; border: 1px solid #007bff;">
				<div class="card-header text-white py-3" style="background: linear-gradient(135deg, #0056b3, #007bff);">
					<h5 class="card-title text-white mb-1"><i class="fas fa-building mr-2"></i> Company Profile</h5>
					<small class="text-white-50 font-weight-normal d-block">Update pharmaceutical company name and corporate details below.</small>
				</div>
						...
						<div class="col-lg-6">
							<div class="form-group mb-0">
								...
								<div class="custom-autocomplete-wrapper position-relative">
									<input class="form-control" type="text" name="name" id="company_name_input" autocomplete="off" placeholder="Search or type company name..." required value="{{ old('name', $company->name) }}">
					...
					<div class="row">
						<div class="col-lg-12">
							<div class="form-group mb-0">
								...
								<textarea class="form-control service-desc" name="description" rows="4" style="resize: vertical;" placeholder="Enter description...">{{ old('description', $company->description) }}</textarea>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="card mb-4 shadow-sm" style="border-radius: 12px; border: 1px solid #e3e8ee;">
				<div class="card-body p-3 d-flex justify-content-between align-items-center">
					<a href="{{route('companies.index')}}" class="btn btn-secondary btn-lg rounded-pill px-4"><i class="fas fa-arrow-left mr-1"></i> Cancel</a>
					<button class="btn btn-primary btn-lg rounded-pill px-5 shadow-sm" type="submit"><i class="fas fa-sync-alt mr-1"></i> Update Company</button>
				</div>
			</div>
		</form>
	</div>			
