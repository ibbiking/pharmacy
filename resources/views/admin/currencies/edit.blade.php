        ...
        <form method="post" action="{{ route('currencies.update', $currency->id) }}">
            ...
            <div class="card mb-4 shadow-sm" style="border-radius: 12px; overflow: hidden; border: 1px solid #007bff;">
                <div class="card-header text-white py-3" style="background: linear-gradient(135deg, #0056b3, #007bff);">
                    ...
                    <small class="text-white-50 font-weight-normal d-block">
                        {{ $currency->isGlobal() ? 'This is a global currency shared by every business.' : 'This currency is only visible to your business.' }}
                    </small>
                </div>
                    ...
                    <div class="row">
                        <div class="col-lg-3">
                            <div class="form-group mb-3">
                                ...
                                <input class="form-control" type="text" name="currency_code" maxlength="10" value="{{ old('currency_code', $currency->currency_code) }}" required style="text-transform: uppercase;">
                            </div>
                        </div>
                        <div class="col-lg-5">
                            <div class="form-group mb-3">
                                <label class="font-weight-bold">Name <span class="text-danger">*</span></label>
                                <input class="form-control" type="text" name="name" value="{{ old('name', $currency->name) }}" required>
                            </div>
                        </div>
                        <div class="col-lg-2">
                            <div class="form-group mb-3">
                                <label class="font-weight-bold">Symbol <span class="text-danger">*</span></label>
                                <input class="form-control" type="text" name="symbol" value="{{ old('symbol', $currency->symbol) }}" required>
                            </div>
                        </div>
                        <div class="col-lg-2">
                            <div class="form-group mb-0">
                                <label class="font-weight-bold">Exchange Rate</label>
                                <input class="form-control" type="number" step="0.000001" min="0" name="exchange_rate" value="{{ old('exchange_rate', $currency->exchange_rate) }}" required>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card mb-4 shadow-sm" style="border-radius: 12px; border: 1px solid #e3e8ee;">
                <div class="card-body p-3 d-flex justify-content-between align-items-center">
                    ...
                    <button class="btn btn-success btn-lg rounded-pill px-5 shadow-sm" type="submit"><i class="fas fa-check-circle mr-1"></i> Update Currency</button>
                </div>
            </div>
        </form>
    </div>
